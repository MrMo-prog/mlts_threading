...
model {
  ...
  for(pp in 1:N){
    vector[N_obs_id[pp]] y_cen;
    y_cen = segment(y[1,],pos_start[pp],N_obs_id[pp]);
    y_cen = y_cen - b_free[pp,1];
    
    vector[N_obs_id[pp]-1] mus;
    mus = b_free[pp,2] * y_cen[1:(N_obs_id[pp]-1)];
    target += normal_lpdf(y_cen[2:N_obs_id[pp]] | mus, sd_noise[pp]);
  }
  ...
  for(pp in 1:N){
    target += multi_normal_cholesky_lpdf(b_free[pp,] | gammas, SIGMA);
  }
